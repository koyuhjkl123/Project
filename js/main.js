...
window.addEventListener('scroll', _.throttle(function () {
  if (window.scrollY > 900) { // 해당 스크롤 내린 위치값이 700보다 크다면
...
window.addEventListener('scroll', _.throttle(function () {
  if (window.scrollY < 400) { // 해당 스크롤 내린 위치값이 700보다 크다면
...
const imageblock = document.querySelector('.notice .notice-line .inner .inner__right .image-block');
...
promotionToggleBtn.addEventListener('click', function () {
  ...
  if(isHidePromotion) {
      ...
      imageClose.classList.add('hide');
  }else {
      // 보이기 처리
      promotionEl.classList.remove('hide');
      imageblock.classList.add('hide');
      imageClose.classList.remove('hide');
  }
});
